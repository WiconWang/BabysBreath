...
class Comm_ImageManager {
    ...
    public static function checkImageMemoryLimit($image) {
        $infos = @getimagesize($image);
        $memory_limit = Comm_Tools::getMemoryLimit();
    ...
    public static function square($src_file, $dest_file, $dest_type, $limit_min = 400, $limit_max = 800) {
        $tmp = tempnam(Comm_Tools::sys_get_temp_dir(), 'img_');
        if (Comm_Tools::saveFile($src_file, $tmp))
        {
    ...
    public static function validateUpload($file, $max_file_size = 0) {
        if ((int)$max_file_size > 0 && $file['size'] > (int)$max_file_size)
            return sprintf(Comm_Tools::displayError('Image is too large (%1$d kB). Maximum allowed: %2$d kB'), $file['size'] / 1024, $max_file_size / 1024);
        ...
        if ($file['error'])
            return sprintf(Comm_Tools::displayError('Error while uploading image; please change your server\'s settings. (Error code: %s)'), $file['error']);
        return true;
    }
    /**
     * 生成图片resource
