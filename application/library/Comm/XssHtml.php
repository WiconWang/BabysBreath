...
class Comm_XssHtml {
	...
	private function __node_img($node){
        ...
        public function setAllowHref($array=array()){
            $this->m_href = is_array($array) ? $array : array();
            return $this;
        }
        private function __sina_a_href($href){
            if(empty($this->m_href)){
                return true;
            }
            $m = parse_url($href);
            if(empty($m['host'])){
            ...
            $res = false;
            foreach($this->m_href as $v){
                $r = substr($m['host'],strlen($v)*(-1)); 
                if($r === $v){
                    $res = true;
                    break;
                }
	...
	private function __node_a($node){
		...
		$href = $this->__get_link($node, 'href');
                $isAllow = $this->__sina_a_href($href);
                if(!$isAllow){
