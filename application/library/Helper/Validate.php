...
class Helper_Validate {
	public static function isEmail($email) {
		return !empty($email) && preg_match(Helper_Tools::cleanNonUnicodeSupport('/^[a-z\p{L}0-9!#$%&\'*+\/=?^`{}|~_-]+[.a-z\p{L}0-9!#$%&\'*+\/=?^`{}|~_-]*@[a-z\p{L}0-9]+[._a-z\p{L}0-9-]*\.[a-z0-9]+$/ui'), $email);
	}
	...
	public static function isOptFloat($float) {
		return empty($float) || Helper_Validate::isFloat($float);
	}

	public static function isName($name) {
		return preg_match(Helper_Tools::cleanNonUnicodeSupport('/^[^!<>,;?=+()@#"°{}$%:]+$/u'), stripslashes($name));
	}
	...
	public static function isPasswd($passwd, $size = 6) {
		return preg_match('/^[.A-Za-z_0-9-!@#$%\^&*()]{' . $size . ',20}$/ui', $passwd);
	}
	/*必须要含有数字有字母 6~20位*/
	public static function isPasswd2($passwd, $size = 6) {
		return preg_match('/(?!^[0-9]*$)(?!^[a-zA-Z]*$)^([a-zA-Z0-9]{' . $size . ',20})$/ui', $passwd);
	}
	...
	public static function isPercentage($value) {
		return (Helper_Validate::isFloat($value) && $value >= 0 && $value <= 100);
	}

	public static function isUnsignedId($id) {
		return Helper_Validate::isUnsignedInt($id); /* Because an id could be equal to zero when there is no association */
	}

	public static function isNullOrUnsignedId($id) {
		return $id === null || Helper_Validate::isUnsignedId($id);
	}
