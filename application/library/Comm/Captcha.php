...
class Comm_Captcha {
     ...
     * @param int $font_size
     */
    public static function generate($len, $width = 108, $height = 50, $font_size = 26,$back='red') {
        $sizes = array('26' => array('width' => 15, 'height' => 42));
        $words = self::words($len);
        session_start();
        $session_key = 'captcha';
        $_SESSION[$session_key] = strtolower($words);
        //生成 Cookie 验证 请同时核验 session 提交数据 和 cookie
        setcookie($session_key, Comm_Tools::Captcha_md5($_SESSION[$session_key]), time()+120,'/');
        $image = Comm_ImageManager::createWhiteImage($width, $height);
        $font_config = array('spacing' => 0, 'font' => 'Ed_Gein_Ynnocent.otf');
        $font_path = dirname(__FILE__) . '/font/' . $font_config['font'];
        
        if($back=='yellow'){
        	$color = imagecolorallocate($image, 253, 217, 13);
