...
class Helper_Tools {
	...
	public static function getCurrentUrlProtocolPrefix() {
		if (Helper_Tools::usingSecureMode ())
			return 'https://';
		else
			return 'http://';
	}
	...
	public static function secureReferrer($referrer) {
		if (preg_match ( '/^http[s]?:\/\/' . Helper_Tools::getServerName () . '(:443)?\/.*$/Ui', $referrer ))
			return $referrer;
	...
	public static function safeOutput($string, $html = false) {
		...
		return @Helper_Tools::htmlentitiesUTF8 ( $string, ENT_QUOTES );
	}
			...
			foreach ( $files as $file )
				if ($file != '.' && $file != '..' && $file != '.svn') {
					if (is_dir ( $dirname . $file ))
						Helper_Tools::deleteDirectory ( $dirname . $file, true );
	...
	public static function getToken($string) {
		return ! empty ( $string ) ? Helper_Tools::encrypt ( $string ) : false;
	}
	...
	public static function truncate($str, $max_length, $suffix = '...') {
		if (Helper_Tools::strlen ( $str ) <= $max_length)
			return $str;
		$str = utf8_decode ( $str );

		return (utf8_encode ( substr ( $str, 0, $max_length - Helper_Tools::strlen ( $suffix ) ) . $suffix ));
	}
	...
	public static function substr($str, $start, $length = false, $encoding = 'utf-8') {
		...
		return substr ( $str, $start, ($length === false ? Helper_Tools::strlen ( $str ) : intval ( $length )) );
	}
	...
	public static function getMaxUploadSize($max_size = 0) {
		$post_max_size = Helper_Tools::convertBytes ( ini_get ( 'post_max_size' ) );
		$upload_max_filesize = Helper_Tools::convertBytes ( ini_get ( 'upload_max_filesize' ) );
		if ($max_size > 0)
	...
	public static function getMemoryLimit() {
		...
		return Helper_Tools::getOctets ( $memory_limit );
	}
	...
	public static function redirectTo($link) {
		if (strpos ( $link, 'http' ) !== false) {
			...
			header ( 'Location: ' . Helper_Tools::getHttpHost ( true ) . '/' . $link );
		}
		exit ();
	}
	...
	public static function file_get_contents($url, $use_include_path = false, $stream_context = null, $curl_timeout = 8) {
		...
		elseif (function_exists ( 'curl_init' )) {
			...
			if (isset ( $opts ['http'] ['method'] ) && Helper_Tools::strtolower ( $opts ['http'] ['method'] ) == 'post') {
	...
	public static function scandir($path, $ext = 'php', $dir = '', $recursive = false) {
		...
		foreach ( $files as $file ) {
			...
			if ($recursive && $file [0] != '.' && is_dir ( $real_path . $file ))
				foreach ( Helper_Tools::scandir ( $path, $ext, $subdir . $file, $recursive ) as $subfile )
				...
				if (! empty ( $postFields )) {
					if (is_array ( $postFields ) || is_object ( $postFields )) {
						if (is_object ( $postFields ))
							$postFields = Helper_Tools::object2array ( $postFields );
						$postBodyString = "";
						$postMultipart = false;
	...
	public static function saveFile($url, $filepath) {
		if (Helper_Validate::isAbsoluteUrl ( $url ) && ! empty ( $filepath )) {
